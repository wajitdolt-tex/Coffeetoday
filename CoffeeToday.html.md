...
.badge-phday{background:#fef3c7;color:#92400e}.badge-late{background:#fee2e2;color:#991b1b}
...
.loading-spinner{display:inline-block;width:16px;height:16px;border:2.5px solid rgba(255,255,255,.3);border-top-color:#fff;border-radius:50%;animation:spin .8s linear infinite;vertical-align:middle;margin-right:6px}
...
@media(max-width:640px){.page{padding:14px}.clock-time{font-size:36px}.form-row,.form-row.three{grid-template-columns:1fr}.stats-row{grid-template-columns:repeat(2,1fr)}.topbar{padding:0 12px;gap:8px}.topbar-brand{font-size:15px}}
</style>
</head>
<body>
<div class="pin-screen hidden" id="pinScreen">
  ...
  <input class="pin-input" id="pinInput" type="password" maxlength="4" placeholder="••••" oninput="document.getElementById('pinError').textContent='';this.classList.remove('error')" onkeydown="if(event.key==='Enter')checkPin()">
...
<div class="sync-bar" id="syncBar"><span class="sync-dot" id="syncDot"></span><span id="syncText">กำลังเชื่อมต่อ...</span></div>
...
<div class="page active" id="page-clock">
  ...
  <div class="comm-card" id="commCard" style="display:none">
    ...
    <div id="commTierInfo" style="font-size:12px;color:#92400e;background:#fff7ed;border-radius:8px;padding:10px 12px;margin-bottom:14px;line-height:1.8"></div>
    <div class="form-row">
      <div class="form-group"><label>วันที่ (ย้อนหลังได้)</label><input type="date" id="commDate"></div>
    ...
    <div id="commCalcResult" style="display:none;background:#f0fdf4;border:2px solid #86efac;border-radius:10px;padding:14px;margin-bottom:14px;text-align:center">
      <div style="font-size:13px;color:var(--muted)">ยอดขาย <strong id="commSalesDisplay">0</strong> บาท · ส่วนต่างจากขั้นต่ำ <strong id="commDiffDisplay">0</strong> บาท × <strong id="commRateDisplay">0%</strong></div>
      <div id="commCalcFormula" style="font-size:12px;color:var(--muted);margin-top:3px"></div>
      <div style="font-family:'Prompt',sans-serif;font-size:28px;font-weight:700;color:var(--success);margin-top:6px">ค่าคอม ฿<span id="commAmountDisplay">0</span></div>
    </div>
    <div id="commNoTier" style="display:none;background:#fef9c3;border:1.5px solid #fcd34d;border-radius:8px;padding:10px 12px;font-size:13px;color:#854d0e;margin-bottom:14px">⚠️ ยอดขายยังไม่ถึงขั้นต่ำ</div>
  ...
  <div class="send-card" id="sendHRCard" style="display:none">
    ...
    <div class="form-group" style="max-width:300px;margin-bottom:14px"><label>เลือกรอบเงินเดือน</label><select id="empSendPeriod"></select></div>
  ...
  <div class="card">
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>วันที่</th><th>สาขา</th><th>เข้า</th><th>สถานะ</th><th>ออก</th><th>ชม.ปกติ</th><th>OT</th><th>ประเภท</th><th>ยอดขาย/คอม</th><th>หมายเหตุ</th></tr></thead>
        <tbody id="myLogTable"><tr><td colspan="10" style="text-align:center;padding:20px;color:var(--muted)">กำลังโหลด...</td></tr></tbody>
      </table>
    </div>
  </div>
</div>
...
<div class="page" id="page-hr">
  ...
  <div class="tab-content" id="tab-allLogs">
    ...
    <div class="card"><div class="card-title">📋 รายการบันทึกเวลา</div>
      <div class="table-wrap"><table>
        <thead><tr><th>วันที่</th><th>สาขา</th><th>พนักงาน</th><th>เข้า</th><th>สถานะ</th><th>ออก</th><th>ชม.</th><th>OT</th><th>ประเภท</th><th>ยอดขาย/คอม</th><th>หมายเหตุ</th><th>ลบ</th></tr></thead>
    ...
    <div class="card"><div class="card-title">✏️ บันทึกเวลาด้วยตนเอง (HR)</div>
      ...
      <div class="form-row">
        <div class="form-group"><label>ประเภท</label>
          <select id="manType"><option value="normal">ปกติ (OT อัตโนมัติ)</option><option value="holiday_ot">OT วันหยุด</option><option value="public_holiday">วันนักขัตฤกษ์</option><option value="holiday_work">ทำงานวันหยุด ×2</option></select></div>
  ...
  <div class="tab-content" id="tab-holidays">
    <div class="card"><div class="card-title">📅 วันหยุดนักขัตฤกษ์ ปี 2569</div>
      ...
      <div class="table-wrap"><table><thead><tr><th>#</th><th>วันที่</th><th>วันหยุด</th></tr></thead><tbody id="phTable"></tbody></table></div>
    </div>
  </div>
</div>
...
<div class="modal-overlay" id="viewSubmissionModal">
  <div class="modal wide">
    ...
    <div class="modal-actions"><button class="btn btn-ghost" onclick="closeModal('viewSubmissionModal')">ปิด</button></div>
  </div>
</div>
<div id="toast"></div>

<script>
// API URL — ฝังไว้แล้ว ไม่ต้องแก้
const API_URL='https://script.google.com/macros/s/AKfycbw4fXghaBjjU_9PvBCFmI14kGROL_rWOp-AyqJ09HAtzZZz6EgXtWf5SY-5rukLJnoyfA/exec';
...
const BRANCH_SCHEDULE={
  lotus:  {start:'09:00',hours:9},
  passion:{start:'10:00',hours:9}
};
function getSched(branch){return BRANCH_SCHEDULE[branch]||{start:'09:00',hours:9};}

// คำนวณนาทีที่สาย (0 ถ้าตรงเวลาหรือมาก่อน)
...
// คำนวณชั่วโมงปกติและ OT จากเวลาเข้า-ออก
// ถ้าสาย → ไม่มี OT, นับชั่วโมงทำงานจริงทั้งหมดเป็นปกติ
// ถ้าตรงเวลา → OT เริ่มหลัง start+hours ชั่วโมง
function calcOT(branch,clockIn,clockOut){
  ...
  var totalMins=om-im;
  var hoursOT=0,hours=0;
  if(late>0){
    // มาสาย: ไม่มี OT
    hours=+(totalMins/60).toFixed(2);
    hoursOT=0;
  } else {
    // ตรงเวลา: OT เริ่มหลัง start + hours
    var otStartMins=sm+(s.hours*60);
    var otMins=Math.max(0,om-otStartMins);
    var normMins=totalMins-otMins;
    hours=+(normMins/60).toFixed(2);
    hoursOT=+(otMins/60).toFixed(2);
  }
  return{hours:Math.max(0,hours),hoursOT:Math.max(0,hoursOT),lateMinutes:late};
}

// อัตราค่าคอม: คิดจาก (ยอดขาย - ขั้นต่ำของ tier) × อัตรา
const COMM_TIERS={
...
function getCommTier(branch,sales){var t=COMM_TIERS[branch]||[];for(var i=0;i<t.length;i++){if(sales>=t[i].min)return t[i];}return null;}
...
function getTierInfoHTML(branch){
  ...
  var rows=[...t].reverse().map(function(x){return'<span style="margin-right:10px">≥฿'+x.min.toLocaleString()+' → ส่วนต่าง×<strong>'+x.label+'</strong></span>';}).join('');
  return'📊 '+rows+'<br><span style="font-size:11px;color:#b45309">เวลาเข้างาน: '+sched.start+' | ปกติ '+sched.hours+' ชม. แล้ว OT</span>';
}
...
async function api(action,data={}){
  ...
  try{
    ...
    var isWrite=['addLog','updateLog','deleteLog','saveEmployee','deleteEmployee','addSubmission','updateSubmission'].includes(action);
    var resp;
    if(isWrite){resp=await fetch(url,{method:'POST',body:JSON.stringify({action,...data})});}
    else{var qs=Object.entries(data).map(([k,v])=>k+'='+encodeURIComponent(v)).join('&');resp=await fetch(url+(qs?'&'+qs:''));}
    var json=await resp.json();
    if(json.error)throw new Error(json.error);
    setSyncStatus('ok','เชื่อมต่อแล้ว · '+new Date().toLocaleTimeString('th-TH'));
    return json;
  }catch(e){setSyncStatus('error','เชื่อมต่อล้มเหลว: '+e.message);throw e;}
}
async function loadAll(){
  try{
    var[eRes,lRes,sRes]=await Promise.all([api('getEmployees'),api('getLogs'),api('getSubmissions')]);
...
function checkPin(){
  ...
  else{var i=document.getElementById('pinInput');i.classList.add('error');document.getElementById('pinError').textContent='รหัสไม่ถูกต้อง';i.value='';setTimeout(()=>i.classList.remove('error'),400);}
}
...
function updateBranchUI(){var b=document.getElementById('heroBranchBadge'),n=document.getElementById('heroBranchName');if(currentBranch&&BRANCHES[currentBranch]){b.style.display='inline-flex';n.textContent=BRANCHES[currentBranch];}else b.style.display='none';}
function brL(b){return BRANCHES[b]||b||'—';}

// PAY PERIODS
function getPayPeriods(){
  var seen=new Set(),periods=[];
  var fixed={key:'2026-03',label:'16 มี.ค. 2569 – 15 เม.ย. 2569',start:new Date(2026,2,16),end:new Date(2026,3,15)};
  periods.push(fixed);seen.add('2026-03');
  var now=new Date();
  for(var i=-1;i<=13;i++){
    var d=new Date(now.getFullYear(),now.getMonth()-i,1);
    var y=d.getFullYear(),m=d.getMonth(),key=y+'-'+String(m+1).padStart(2,'0');
    if(seen.has(key))continue;seen.add(key);
    var s=new Date(y,m,16),e=new Date(y,m+1,15);
    periods.push({key:key,label:s.toLocaleDateString('th-TH',{day:'numeric',month:'short',year:'numeric'})+' – '+e.toLocaleDateString('th-TH',{day:'numeric',month:'short',year:'numeric'}),start:s,end:e});
  }
...
function populatePPSelects(){
  ...
  var opts=periods.map(p=>'<option value="'+p.key+'"'+(p.key===cur?' selected':'')+'>'+p.label+'</option>').join('');
  ['filterPayPeriod','summaryPayPeriod'].forEach(id=>{var el=document.getElementById(id);if(el)el.innerHTML=opts;});
  var ep=document.getElementById('empSendPeriod');if(ep)ep.innerHTML=opts;
}
...
function updateClock(){
  ...
  var ph=getPH(now.toISOString().split('T')[0]),el=document.getElementById('phToday');
...
function renderEmpCards(){
  var c=document.getElementById('empCards'),active=employees.filter(e=>e.active);
  if(!active.length){c.innerHTML='<p style="color:var(--muted);font-size:13px">ยังไม่มีพนักงาน หรือกำลังโหลด...</p>';return;}
  c.innerHTML=active.map(e=>'<div class="emp-card '+(selectedEmpId===e.id?'selected':'')+'" onclick="selectEmp(\''+e.id+'\',this)"><div class="emp-avatar">'+e.name.charAt(0)+'</div><div class="emp-name">'+e.name+'</div><div style="font-size:10px;color:var(--muted);margin-top:2px">'+brL(e.branch)+'</div></div>').join('');
}
function selectEmp(id,el){
  ...
  // ค่าคอมย้อนหลังได้ ไม่ lock วันที่
...
function checkPHWarn(){var ph=getPH(new Date().toISOString().split('T')[0]),w=document.getElementById('phWarning');if(ph){w.style.display='block';document.getElementById('phWarningName').textContent=ph.name;}else w.style.display='none';}
...
function updateCOPreview(){
  ...
  var sched=getSched(branch);
  var r=calcOT(branch,entry.clockIn,ts);
  var lateText=r.lateMinutes>0
    ?'<span style="color:#ef4444;font-weight:600"> ⚠️ สาย '+r.lateMinutes+' นาที (ไม่มี OT)</span>'
    :'<span style="color:#10b981;font-weight:600"> ✅ ตรงเวลา</span>';
  var otText=r.hoursOT>0?' | OT: <strong style="color:#b45309">'+r.hoursOT+' ชม.</strong>':'';
  box.style.display='block';
  box.innerHTML='เข้างาน <strong>'+entry.clockIn+'</strong> (มาตรฐาน '+sched.start+')'+lateText+'<br>ปกติ: <strong>'+r.hours+' ชม.</strong>'+otText;
}

async function submitClock(){
  var empId=document.getElementById('modalEmp').value;
  if(!empId){showToast('กรุณาเลือกพนักงาน','error');return;}
  var now=new Date(),ds=now.toISOString().split('T')[0],ts=now.toTimeString().slice(0,5),ph=getPH(ds);
  var btn=document.getElementById('clockModalBtn');
  ...
  try{
    if(clockAction==='in'){
      ...
      var sched=getSched(branch);
      var late=calcLate(branch,ts);
      var lateMsg=late>0?' ⚠️ สาย '+late+' นาที (มาตรฐาน '+sched.start+')':'';
      var log={id:genId(),empId:empId,date:ds,branch:branch,clockIn:ts,clockOut:'',
        type:ph?'public_holiday':'normal',sales:'',commission:0,commRate:0,
        note:document.getElementById('modalNote').value,hours:'',hoursOT:0,lateMinutes:late};
      ...
      var hours2,hoursOT2,type2,late2=0;
      var sp=['holiday_ot','public_holiday','holiday_work'].includes(entry.type);
      if(sp){
        hours2=+hRaw(entry.clockIn,ts).toFixed(2);hoursOT2=0;type2=entry.type;
      } else {
        var r=calcOT(branch2,entry.clockIn,ts);
        hours2=r.hours;hoursOT2=r.hoursOT;late2=r.lateMinutes;
        type2=hoursOT2>0?'split':entry.type;
      }
      await api('updateLog',{id:entry.id,clockOut:ts,hours:hours2,hoursOT:hoursOT2,type:type2,note:note,lateMinutes:late2});
      Object.assign(entry,{clockOut:ts,hours:hours2,hoursOT:hoursOT2,type:type2,note:note,lateMinutes:late2});
      var lateMsg2=late2>0?' (สาย '+late2+' นาที)':'';
      var otMsg=hoursOT2>0?' OT '+hoursOT2+' ชม.':'';
      showToast('🚪 ออกงาน'+lateMsg2+otMsg,'success');
    }
...
function onSalesInput(){
  var sales=parseFloat(document.getElementById('commSales').value)||0;
  var emp=employees.find(e=>e.id===selectedEmpId);
  var branch=emp?emp.branch:currentBranch;
  var calc=document.getElementById('commCalcResult'),noT=document.getElementById('commNoTier');
  if(sales<=0){calc.style.display='none';noT.style.display='none';return;}
  var r=calcCommission(branch,sales);
  if(r.tier){
...
async function saveCommission(){
  ...
  var date=document.getElementById('commDate').value;
  if(!date){showToast('กรุณาเลือกวันที่','error');return;}
  var emp=employees.find(e=>e.id===selectedEmpId);
  var branch=emp?emp.branch:currentBranch;
  var r=calcCommission(branch,sales);
  var note=document.getElementById('commNote').value||(r.tier?'ยอด ฿'+sales.toLocaleString()+' ส่วนต่าง ฿'+r.diff.toLocaleString()+'×'+r.tier.label:'ยอดขาย ฿'+sales.toLocaleString());
...
async function openSendSummaryModal(){
  if(!selectedEmpId){showToast('กรุณาเลือกพนักงาน','error');return;}
  var ppKey=document.getElementById('empSendPeriod').value;
  if(!ppKey){showToast('กรุณาเลือกรอบ','error');return;}
  var emp=employees.find(e=>e.id===selectedEmpId);if(!emp)return;
  var pl=logsInPP(ppKey).filter(l=>l.empId===selectedEmpId);
  document.getElementById('sendSummaryContent').innerHTML=buildSummaryHTML(emp,pl,ppKey,false);
  window._pendingSend={empId:selectedEmpId,ppKey:ppKey};
  openModal('sendSummaryModal');
}
async function confirmSendSummary(){
  var ps=window._pendingSend||{};if(!ps.empId||!ps.ppKey)return;
  var emp=employees.find(e=>e.id===ps.empId);
  var pl=logsInPP(ps.ppKey).filter(l=>l.empId===ps.empId);
  var norm=pl.filter(l=>l.type!=='holiday_work').reduce((s,l)=>s+(Number(l.hours)||0),0);
  var ot=pl.reduce((s,l)=>s+(Number(l.hoursOT)||0),0);
  var comm=pl.reduce((s,l)=>s+(Number(l.commission)||0),0);
  var hwDays=new Set(pl.filter(l=>l.type==='holiday_work').map(l=>l.date)).size;
  var sub={id:genId(),empId:ps.empId,empName:emp.name,branch:emp.branch,ppKey:ps.ppKey,sentAt:new Date().toISOString(),status:'pending',norm:norm,ot:ot,comm:comm,hwDays:hwDays};
  try{await api('addSubmission',sub);submissions.push(sub);closeModal('sendSummaryModal');showToast('📤 ส่งสรุปให้ HR สำเร็จ','success');}
  catch(e){showToast('ส่งล้มเหลว: '+e.message,'error');}
}

// MY LOG
function renderMyLog(){
  var tbody=document.getElementById('myLogTable');
  var myLogs=selectedEmpId?[...logs].filter(l=>l.empId===selectedEmpId).reverse().slice(0,50):[...logs].reverse().slice(0,50);
...
function populateFilterSelects(){
  ['filterEmp','summaryEmp'].forEach(function(id){var sel=document.getElementById(id);if(!sel)return;sel.innerHTML='<option value="">ทั้งหมด</option>'+employees.map(e=>'<option value="'+e.id+'">'+e.name+'</option>').join('');});
}
function renderDashboard(){
  ...
  document.getElementById('empSummaryTable').innerHTML=employees.filter(e=>e.active).map(function(e){
    var el=pl.filter(l=>l.empId===e.id);
    var totalSales=el.filter(l=>l.type==='commission').reduce((s,l)=>s+(Number(l.sales)||0),0);
    var comm=el.reduce((s,l)=>s+(Number(l.commission)||0),0);
...
function renderAllLogs(){
  var empF=document.getElementById('filterEmp').value,branchF=document.getElementById('filterBranch').value,ppKey=document.getElementById('filterPayPeriod').value;
  var eMap={};employees.forEach(e=>eMap[e.id]={name:e.name,branch:e.branch});
  var filtered=[...logs].reverse();
  ...
  if(ppKey){var pids=new Set(logsInPP(ppKey).map(l=>l.id));filtered=filtered.filter(l=>pids.has(l.id));}
...
async function deleteLogHR(id){
  ...
  try{await api('deleteLog',{id:id});logs=logs.filter(l=>l.id!==id);renderAllLogs();renderDashboard();showToast('ลบเรียบร้อย');}
  catch(e){showToast('ลบล้มเหลว: '+e.message,'error');}
}
function buildSummaryHTML(emp,el,ppKey,showSent){
  var norm=el.filter(l=>l.type!=='holiday_work').reduce((s,l)=>s+(Number(l.hours)||0),0);
  ...
  return '<div class="summary-report">'+
    ...
    '<div style="font-size:12px;color:var(--muted)">รอบ: '+getPPLabel(ppKey)+' | เวลาเข้างาน: '+sched.start+' | OT หลัง '+sched.hours+' ชม.</div>'+
...
function renderSummary(){
  ...
  var pl=logsInPP(ppKey);
  var targets=employees.filter(e=>e.active);
...
async function renderSubmissions(){
  ...
  tbody.innerHTML=[...submissions].reverse().map(function(s){
    return '<tr>'+
      ...
      '<td style="display:flex;gap:5px"><button class="btn btn-dark btn-sm" onclick="viewSubmission(\''+s.id+'\')">📋</button>'+(s.status==='pending'?'<button class="btn btn-success btn-sm" onclick="approveSubmission(\''+s.id+'\')">✓</button>':'')+  '</td></tr>';
  }).join('');
}
function viewSubmission(id){
  var s=submissions.find(x=>x.id===id);if(!s)return;
  var emp=employees.find(e=>e.id===s.empId);if(!emp)return;
  var pl=logsInPP(s.ppKey).filter(l=>l.empId===s.empId);
...
async function approveSubmission(id){
  try{await api('updateSubmission',{id:id,status:'approved'});var s=submissions.find(x=>x.id===id);if(s)s.status='approved';renderSubmissions();showToast('✅ รับทราบแล้ว','success');}
  catch(e){showToast('ล้มเหลว: '+e.message,'error');}
}
function renderEmpTable(){
  document.getElementById('empTable').innerHTML=employees.map(function(e){
    return '<tr><td><strong>'+e.name+'</strong></td><td style="font-size:11px;color:var(--mid);font-weight:600">'+brL(e.branch)+'</td>'+
...
async function toggleEmpStatus(id){var e=employees.find(x=>x.id===id);if(!e)return;e.active=!e.active;try{await api('saveEmployee',Object.assign({},e));renderEmpTable();renderEmpCards();}catch(err){e.active=!e.active;showToast('ล้มเหลว: '+err.message,'error');}}
async function deleteEmp(id){if(!confirm('ลบพนักงานนี้?'))return;try{await api('deleteEmployee',{id:id});employees=employees.filter(e=>e.id!==id);renderEmpTable();renderEmpCards();showToast('ลบเรียบร้อย');}catch(e){showToast('ลบล้มเหลว: '+e.message,'error');}}
...
async function saveEmployee(){
  var name=document.getElementById('newEmpName').value.trim();if(!name){showToast('กรุณากรอกชื่อ','error');return;}
  var editId=document.getElementById('editEmpId').value;
  var data={id:editId||'emp'+Date.now(),name:name,branch:document.getElementById('newEmpBranch').value||'lotus',
  ...
  try{
    ...
    if(editId){var idx=employees.findIndex(e=>e.id===editId);if(idx>-1)employees[idx]=Object.assign({},employees[idx],data);}
...
async function saveManual(){
  var empId=document.getElementById('manEmp').value,date=document.getElementById('manDate').value;
  var inT=document.getElementById('manIn').value,outT=document.getElementById('manOut').value;
  var type=document.getElementById('manType').value,branch=document.getElementById('manBranch').value;
  var salesVal=parseFloat(document.getElementById('manSales').value)||0;
  if(!empId||!date||!inT){showToast('กรุณากรอกข้อมูลให้ครบ','error');return;}
  var hours=null,hoursOT=0,lateMinutes=0;
  if(outT){
    var sp=['holiday_ot','public_holiday','holiday_work'].includes(type);
    if(sp){hours=+hRaw(inT,outT).toFixed(2);hoursOT=0;}
    else{var r2=calcOT(branch,inT,outT);hours=r2.hours;hoursOT=r2.hoursOT;lateMinutes=r2.lateMinutes;}
  }
  var ft=(hours!==null&&hoursOT>0&&!['holiday_ot','public_holiday','holiday_work'].includes(type))?'split':type;
  var rc=salesVal>0?calcCommission(branch,salesVal):{commission:0,tier:null,diff:0};
  var log={id:genId(),empId:empId,date:date,branch:branch,clockIn:inT,clockOut:outT||'',
    hours:hours||'',hoursOT:hoursOT,type:ft,lateMinutes:lateMinutes,
    sales:salesVal||'',commDiff:rc.diff||'',commission:rc.commission,commRate:rc.tier?rc.tier.rate:0,
...
var toastTimer;
function showToast(msg,type){var t=document.getElementById('toast');t.textContent=msg;t.className='show '+(type||'');clearTimeout(toastTimer);toastTimer=setTimeout(function(){t.className='';},4000);}
document.querySelectorAll('.modal-overlay').forEach(o=>o.addEventListener('click',e=>{if(e.target===o)o.classList.remove('open');}));

// INIT
if(currentBranch)document.getElementById('branchSelect').value=currentBranch;
updateBranchUI();populatePPSelects();loadAll();
</script>
